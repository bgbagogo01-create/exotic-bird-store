...
<?php $__env->startSection('page_title', 'Kelola Barang (Inventaris)'); ?>

<?php $__env->startSection('content'); ?>
<div class="space-y-6" x-data="{ 
    modalOpen: <?php echo e($errors->any() ? 'true' : 'false'); ?>, 
    modalTitle: '<?php echo e(old('_method') === 'PUT' ? 'Edit Data Barang' : 'Tambah Barang Baru'); ?>', 
    isEdit: <?php echo e(old('_method') === 'PUT' ? 'true' : 'false'); ?>, 
    actionUrl: '<?php echo e(old('_method') === 'PUT' ? route('products.update', old('id', 0)) : route('products.store')); ?>', 
    methodType: 'POST', 
    prodId: '<?php echo e(old('id')); ?>', 
    prodName: '<?php echo e(old('name')); ?>', 
    prodSku: '<?php echo e(old('sku')); ?>', 
    prodCat: '<?php echo e(old('category_id')); ?>', 
    prodDesc: '<?php echo e(old('description')); ?>', 
    prodPrice: '<?php echo e(old('price')); ?>', 
    prodStock: '<?php echo e(old('stock')); ?>', 
    prodMinStock: '<?php echo e(old('min_stock')); ?>', 
    oldImage: '<?php echo e(old('old_image')); ?>' 
}">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button @click="modalOpen = true; isEdit = false; modalTitle = 'Tambah Barang Baru'; actionUrl = '<?php echo e(route('products.store')); ?>'; methodType = 'POST'; prodId=''; prodName=''; prodSku=''; prodCat=''; prodDesc=''; prodPrice=''; prodStock=''; prodMinStock=''; oldImage=''" 
    ...
    <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm">
        <form action="<?php echo e(route('products.index')); ?>" method="GET" class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <!-- Search Query -->
            <div class="space-y-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo e(request()->get('search')); ?>" placeholder="Nama, SKU, atau deskripsi..."
            ...
            <div class="space-y-1">
                ...
                <select name="category" class="w-full px-3 py-2 rounded-xl bg-white/50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-850 text-xs focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none transition-all cursor-pointer">
                    ...
                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($cat->id); ?>" <?php echo e(request()->get('category') == $cat->id ? 'selected' : ''); ?>><?php echo e($cat->name); ?></option>
            ...
            <div class="space-y-1">
                ...
                <select name="status" class="w-full px-3 py-2 rounded-xl bg-white/50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-850 text-xs focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none transition-all cursor-pointer">
                    ...
                    <option value="tersedia" <?php echo e(request()->get('status') == 'tersedia' ? 'selected' : ''); ?>>Tersedia</option>
                    <option value="habis" <?php echo e(request()->get('status') == 'habis' ? 'selected' : ''); ?>>Habis / Kosong</option>
                </select>
            </div>
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="<?php echo e(route('products.index')); ?>" class="px-3 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-semibold rounded-xl text-xs transition-all hover:bg-slate-200 dark:hover:bg-slate-700 text-center">
                    Reset
                </a>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60" id="productTableBody">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                        ...
                        <td class="py-3.5 flex items-center space-x-3">
                            <div class="w-10 h-10 rounded-xl overflow-hidden border dark:border-slate-850 shrink-0 bg-slate-100 shadow-sm relative group">
                                <img src="<?php echo e($product->image_url); ?>" alt="Barang" class="w-full h-full object-cover">
                            </div>
                            <div class="min-w-0">
                                <span class="font-bold text-slate-800 dark:text-slate-100 truncate block"><?php echo e($product->name); ?></span>
                                <span class="text-[10px] text-slate-400 truncate block"><?php echo e($product->slug); ?></span>
                            </div>
                        </td>
                        <td class="py-3.5 font-mono text-[10px] font-semibold tracking-wider"><?php echo e($product->sku); ?></td>
                        <td class="py-3.5 text-slate-500 dark:text-slate-400"><?php echo e($product->category->name); ?></td>
                        <td class="py-3.5 font-bold">
                            <?php echo e(\App\Models\Setting::get('shop_currency', 'Rp')); ?> <?php echo e(number_format($product->price, 0, ',', '.')); ?>

                        </td>
                        <td class="py-3.5 text-center font-medium text-slate-400"><?php echo e($product->min_stock); ?> pcs</td>
                        <td class="py-3.5 text-center">
                            <span class="px-2 py-0.5 rounded-full font-bold <?php echo e($product->stock <= $product->min_stock ? 'bg-rose-500/10 text-rose-500' : 'bg-indigo-500/10 text-indigo-500'); ?>">
                        ...
                        <td class="py-3.5 text-right space-x-1.5 whitespace-nowrap">
                            <!-- Edit Button -->
                            <button type="button" @click="modalOpen = true; isEdit = true; modalTitle = 'Edit Data Barang'; actionUrl = '<?php echo e(route('products.update', $product->id)); ?>'; methodType = 'POST'; prodId='<?php echo e($product->id); ?>'; prodName='<?php echo e($product->name); ?>'; prodSku='<?php echo e($product->sku); ?>'; prodCat='<?php echo e($product->category_id); ?>'; prodDesc='<?php echo e($product->description); ?>'; prodPrice='<?php echo e($product->price); ?>'; prodStock='<?php echo e($product->stock); ?>'; prodMinStock='<?php echo e($product->min_stock); ?>'; oldImage='<?php echo e($product->image_url); ?>'" 
                            ...
                            <!-- Delete Form -->
                            <form action="<?php echo e(route('products.destroy', $product->id)); ?>" method="POST" id="delete-form-<?php echo e($product->id); ?>" class="inline-block">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('DELETE'); ?>
                                <button type="button" onclick="confirmDelete('delete-form-<?php echo e($product->id); ?>', '<?php echo e($product->name); ?>')" 
                    ...
                    <tr>
                        <td colspan="<?php echo e(Auth::user()->isAdmin() ? 9 : 8); ?>" class="py-8 text-center text-slate-400 dark:text-slate-500">Tidak ada data produk ditemukan.</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
$message = $__bag->first($__errorArgs[0]); ?>
                            <span class="text-[10px] text-rose-500 font-semibold block mt-1"><?php echo e($message); ?></span>
                        <?php unset($message);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\syarat kp\project kp\resources\views/products/index.blade.php ENDPATH**/ ?>