...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm max-w-3xl mx-auto space-y-6">
        ...
        <form action="{{ route('catalog.index') }}" method="GET" class="relative">
            ...
            <input type="text" name="q" value="{{ request()->get('q') }}" placeholder="Cari barang impian Anda di sini..."
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pt-4">
        ...
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-4 shadow-sm flex flex-col justify-between hover:scale-[1.01] transition-transform duration-250 group">
            ...
            <a href="{{ route('catalog.show', $prod->slug) }}" class="space-y-4">
                <!-- Product Image box -->
                ...
                <div class="space-y-1">
                    ...
                    <h3 class="font-bold text-xs text-slate-800 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-1" title="{{ $prod->name }}">{{ $prod->name }}</h3>
                    <p class="text-[10px] text-slate-400 dark:text-slate-500 line-clamp-2 leading-relaxed font-normal">{{ $prod->description ?: 'Spesifikasi detail produk dapat dilihat di halaman selengkapnya.' }}</p>
                </div>
            </a>
            ...
            <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                <div>
                    ...
                    <span class="font-extrabold text-xs text-slate-800 dark:text-slate-100">
                        {{ \App\Models\Setting::get('shop_currency', 'Rp') }} {{ number_format($prod->price, 0, ',', '.') }}
                    </span>
                </div>
            ...
            <div class="grid grid-cols-2 gap-2 mt-4">
                ...
                @endphp
                <a href="{{ $quickWaLink }}" target="_blank"
        ...
        <div class="col-span-full bg-white/60 dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-12 text-center text-slate-400 dark:text-slate-500 max-w-md mx-auto">
            ...
            <a href="{{ route('catalog.index') }}" class="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-xl text-[10px] uppercase tracking-wider mt-4 inline-block shadow shadow-indigo-600/10">Lihat Semua Produk</a>
        </div>
        @endforelse
    </div>
