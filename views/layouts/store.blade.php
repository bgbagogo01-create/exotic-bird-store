...
<head>
    ...
    <title>@yield('title', 'Katalog Produk') - {{ \App\Models\Setting::get('shop_name', 'EXOTIC BIRD STORE') }}</title>
            ...
            <div class="flex items-center space-x-4">
                ...
                @auth
                    ...
                    @if(Auth::user()->isAdmin() || Auth::user()->isKasir())
                        <a href="{{ route('dashboard') }}" class="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm">
                    ...
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('profile') }}" class="flex items-center space-x-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl text-xs font-semibold transition-all">
                            <img src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&background=6366f1&color=fff' }}" alt="Avatar" class="w-5 h-5 rounded-md object-cover">
                            <span class="max-w-[80px] truncate hidden md:inline">{{ Auth::user()->name }}</span>
                        </a>
                ...
                @else
                    ...
                    <a href="{{ route('login') }}" class="text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 transition-colors">Masuk</a>
                    <a href="{{ route('register') }}" class="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm">Daftar</a>
                @endauth
            </div>
        </div>
    </header>
