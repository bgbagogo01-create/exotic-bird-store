...
@section('page_title', 'Ekspor Laporan & Statistik')
...
<div class="space-y-6">
    ...
    <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm">
        ...
        <form action="{{ route('reports.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <!-- Start Date -->
            <div class="space-y-1">
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="{{ route('reports.index') }}" class="px-3 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-semibold rounded-xl text-xs transition-all hover:bg-slate-200 dark:hover:bg-slate-700 text-center">
                    Reset Bln Ini
                </a>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 class="text-md font-extrabold tracking-tight mt-0.5 heading-font">{{ $shopCurrency }} {{ number_format($salesRevenue, 0, ',', '.') }}</h3>
            </div>
        </div>
        ...
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 class="text-md font-extrabold tracking-tight mt-0.5 heading-font">{{ $salesCount }} Transaksi</h3>
            </div>
        </div>
        ...
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 class="text-md font-extrabold tracking-tight mt-0.5 heading-font">+{{ $stockInCount ?: 0 }} Pcs</h3>
            </div>
        </div>
        ...
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 class="text-md font-extrabold tracking-tight mt-0.5 heading-font">-{{ $stockOutCount ?: 0 }} Pcs</h3>
            </div>
        </div>
    ...
    <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm">
        ...
        <div class="divide-y divide-slate-100 dark:divide-slate-800/60 space-y-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 first:pt-0">
                ...
                <div class="flex items-center space-x-2 shrink-0">
                    <a href="{{ route('reports.pdf', ['report_type' => 'sales', 'start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" 
                    ...
                    <a href="{{ route('reports.excel', ['report_type' => 'sales', 'start_date' => $startDate, 'end_date' => $endDate]) }}" 
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4">
                <div class="space-y-1">
                    ...
                    <p class="text-[10px] text-slate-400">Mencatat barang yang masuk ke gudang, penambahan kuantitas, nama supplier, and catatan log.</p>
                </div>
                <div class="flex items-center space-x-2 shrink-0">
                    <a href="{{ route('reports.pdf', ['report_type' => 'stock_in', 'start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" 
                        class="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider shadow-md shadow-rose-500/10 transition-all flex items-center space-x-1.5">
                        <i class="fa-solid fa-file-pdf"></i> <span>Cetak PDF</span>
                    </a>
                    @if(Auth::user()->isAdmin())
                    <a href="{{ route('reports.excel', ['report_type' => 'stock_in', 'start_date' => $startDate, 'end_date' => $endDate]) }}" 
                        class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider shadow-md shadow-green-600/10 transition-all flex items-center space-x-1.5">
                        <i class="fa-solid fa-file-excel"></i> <span>Ekspor Excel</span>
                    </a>
                    @endif
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4">
                ...
                <div class="flex items-center space-x-2 shrink-0">
                    <a href="{{ route('reports.pdf', ['report_type' => 'stock_out', 'start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" 
                        class="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider shadow-md shadow-rose-500/10 transition-all flex items-center space-x-1.5">
                        <i class="fa-solid fa-file-pdf"></i> <span>Cetak PDF</span>
                    </a>
                    @if(Auth::user()->isAdmin())
                    <a href="{{ route('reports.excel', ['report_type' => 'stock_out', 'start_date' => $startDate, 'end_date' => $endDate]) }}" 
                        class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider shadow-md shadow-green-600/10 transition-all flex items-center space-x-1.5">
                        <i class="fa-solid fa-file-excel"></i> <span>Ekspor Excel</span>
                    </a>
                    @endif
        ...
        @if(!Auth::user()->isAdmin())
        <div class="mt-4 bg-indigo-500/5 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 p-4 rounded-2xl text-xs flex items-center space-x-2">
            <i class="fa-solid fa-circle-info"></i>
            <span>Akun Anda terdaftar sebagai <strong>Kasir</strong>. Cetak PDF tersedia untuk laporan tertulis (tidak dapat diedit). Fitur Ekspor Excel eksklusif untuk level <strong>Admin</strong>.</span>
        </div>
        @endif
    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse text-[11px]">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                        ...
                        <tr>
                            <td class="py-2.5 font-bold text-indigo-600 dark:text-indigo-400">{{ $tr->invoice_number }}</td>
                            <td class="py-2.5 text-slate-500 dark:text-slate-400">{{ $tr->buyer_name }}</td>
                            <td class="py-2.5 text-right font-bold">Rp {{ number_format($tr->total_price, 0, ',', '.') }}</td>
                            <td class="py-2.5 text-right text-slate-400">{{ $tr->created_at->format('d/m/Y') }}</td>
                        </tr>
                        @empty
                        <tr>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                            ...
                            <tr>
                                <td class="py-2.5 font-bold text-slate-800 dark:text-slate-100">
                                    {{ $in->product->name }} <span class="text-[9px] font-mono text-slate-400">({{ $in->product->sku }})</span>
                                </td>
                                ...
                                <td class="py-2.5 text-center font-bold text-green-600 dark:text-green-400">+{{ $in->quantity }} pcs</td>
                                <td class="py-2.5 text-right text-slate-400">{{ $in->date }}</td>
                            </tr>
                            @php $inCount++; @endphp
                            @endif
                            @endforeach
                            ...
                            <tr>
                                <td class="py-2.5 font-bold text-slate-800 dark:text-slate-100">
                                    {{ $out->product->name }} <span class="text-[9px] font-mono text-slate-400">({{ $out->product->sku }})</span>
                                </td>
                                ...
                                <td class="py-2.5 text-center font-bold text-rose-600 dark:text-rose-400">-{{ $out->quantity }} pcs</td>
                                <td class="py-2.5 text-right text-slate-400">{{ $out->date }}</td>
                            </tr>
                            @php $outCount++; @endphp
                            @endif
                            @endforeach

                            @if($stockIns->isEmpty() && $stockOuts->isEmpty())
                            <tr>
