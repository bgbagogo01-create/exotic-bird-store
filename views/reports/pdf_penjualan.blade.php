    ...
    <table class="table-data">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="font-bold" style="color: #4f46e5;">{{ $tr->invoice_number }}</td>
                <td>{{ $tr->created_at->format('d/m/Y H:i') }}</td>
                <td>{{ $tr->user->name }}</td>
                <td>{{ $tr->buyer_name }}</td>
                <td class="text-right">Rp {{ number_format($tr->subtotal, 0, ',', '.') }}</td>
                <td class="text-right" style="color: #dc2626;">Rp {{ number_format($tr->discount, 0, ',', '.') }}</td>
                <td class="text-right font-bold">Rp {{ number_format($tr->total_price, 0, ',', '.') }}</td>
            </tr>
            @php 
    ...
    <table class="summary-box">
        <tbody>
            <tr>
                ...
                <td class="summary-value">Rp {{ number_format($sumSubtotal, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td class="summary-title">Total Potongan Diskon</td>
                <td class="summary-value" style="color:#dc2626;">-Rp {{ number_format($sumDiscount, 0, ',', '.') }}</td>
            </tr>
            <tr class="summary-total">
                <td>Total Bersih (Omset)</td>
                <td class="summary-value">Rp {{ number_format($sumTotal, 0, ',', '.') }}</td>
            </tr>
        </tbody>
    </table>
