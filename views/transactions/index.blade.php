...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a href="{{ route('transactions.pos') }}" class="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs shadow-lg shadow-indigo-600/10 transition-all flex items-center space-x-2 w-fit">
    ...
    <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm">
        <form action="{{ route('transactions.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <!-- Search Query -->
            <div class="space-y-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request()->get('search') }}" placeholder="INV-2026... atau nama..."
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="{{ route('transactions.index') }}" class="px-3 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-semibold rounded-xl text-xs transition-all hover:bg-slate-200 dark:hover:bg-slate-700 text-center">
                    Reset
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-left border-collapse text-xs">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors" x-data="{ expanded: false }">
                        ...
                        <td class="py-3.5 font-medium whitespace-nowrap">{{ $transaction->created_at->isoFormat('D MMM Y, H:i') }}</td>
                        <td class="py-3.5 text-slate-500 dark:text-slate-400">{{ $transaction->user->name }}</td>
                        <td class="py-3.5 font-semibold">{{ $transaction->buyer_name }}</td>
                        <td class="py-3.5 text-right font-medium text-slate-400">Rp {{ number_format($transaction->subtotal, 0, ',', '.') }}</td>
                        <td class="py-3.5 text-right text-rose-500 font-medium">-Rp {{ number_format($transaction->discount, 0, ',', '.') }}</td>
                        <td class="py-3.5 text-right font-bold text-slate-800 dark:text-slate-100">Rp {{ number_format($transaction->total_price, 0, ',', '.') }}</td>
                        <td class="py-3.5 text-right whitespace-nowrap">
                            <!-- Printer Button Trigger (Popup window printable receipt) -->
                            <button type="button" onclick="window.open('{{ route('transactions.receipt', $transaction->id) }}', 'Cetak Struk', 'width=450,height=600,scrollbars=yes')"
                            ...
                            <td colspan="9" class="p-4 border-b border-slate-200 dark:border-slate-800/80">
                                <div class="max-w-3xl pl-12 space-y-3">
                                            ...
                                            <tbody class="divide-y divide-slate-100 dark:divide-slate-850">
                                                ...
                                                <tr>
                                                    <td class="p-3 font-medium flex items-center space-x-2">
                                                        <span class="font-semibold">{{ $item->product ? $item->product->name : 'Barang Dihapus' }}</span>
                                                        <span class="text-[9px] font-mono text-slate-400">({{ $item->product ? $item->product->sku : '-' }})</span>
                                                    </td>
                                                    <td class="p-3 text-center text-slate-500 dark:text-slate-400">Rp {{ number_format($item->price, 0, ',', '.') }}</td>
                                                    <td class="p-3 text-center font-bold text-slate-700 dark:text-slate-300">{{ $item->quantity }} pcs</td>
                                                    <td class="p-3 text-right font-semibold text-slate-800 dark:text-slate-100">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                                    ...
                                    <div class="flex items-center justify-between text-[11px] px-2 text-slate-400">
                                        <span>Jumlah Uang Bayar: <strong>Rp {{ number_format($transaction->pay_amount, 0, ',', '.') }}</strong></span>
                                        <span>Uang Kembali: <strong>Rp {{ number_format($transaction->change_amount, 0, ',', '.') }}</strong></span>
                                    </div>
                                </div>
                            </td>
                        </tr>
