...
@section('title', 'Aplikasi POS (Kasir)')
@section('page_title', 'Aplikasi POS (Kasir)')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6" id="posContainer">
        ...
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
            ...
            <div class="flex items-center space-x-2 w-full sm:w-auto">
                ...
                <select id="posCategoryFilter" class="w-full sm:w-48 px-3 py-2 rounded-xl bg-white/50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-850 text-xs focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none transition-all cursor-pointer">
                    ...
                    @endphp
                    @foreach($extractedCategories as $category)
            ...
            <div class="pos-product-card bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-5 shadow-sm hover:scale-[1.01] transition-transform duration-200 flex flex-col justify-between" 
                ...
                <div>
                    ...
                    <div class="relative w-full h-36 rounded-2xl overflow-hidden border dark:border-slate-850 bg-slate-100 mb-4">
                        <img src="{{ $prod->image_url }}" alt="Product Image" class="w-full h-full object-cover">
                    ...
                    <!-- Category & Title -->
                    <div class="space-y-1">
                        <span class="text-[9px] uppercase font-bold text-indigo-600 dark:text-indigo-400 tracking-wider heading-font">{{ $prod->category->name }}</span>
                        <h3 class="font-bold text-xs text-slate-800 dark:text-slate-100 truncate" title="{{ $prod->name }}">{{ $prod->name }}</h3>
                    </div>
                </div>
                ...
                <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                    <div>
                        ...
                        <span class="font-extrabold text-xs text-indigo-600 dark:text-indigo-400">{{ $shopCurrency }} {{ number_format($prod->price, 0, ',', '.') }}</span>
                    </div>
                    <div class="text-right">
                        <span class="text-[9px] text-slate-400 block uppercase font-semibold">Tersedia</span>
                        <span class="text-xs font-bold text-slate-800 dark:text-slate-100">{{ $prod->stock }} pcs</span>
                    </div>
                </div>

                <button type="button" onclick="addToCart({{ $prod->id }}, '{{ addslashes($prod->name) }}', '{{ $prod->sku }}', {{ $prod->price }}, {{ $prod->stock }})"
    ...
    <div class="lg:col-span-1">
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-sm flex flex-col justify-between sticky top-6">
            ...
            <form action="{{ route('transactions.store') }}" method="POST" id="posCheckoutForm" class="space-y-5">
                @csrf
                <!-- Cart JSON payload -->
                ...
                <div class="border-t border-slate-200/50 dark:border-slate-800/50 pt-4 space-y-2.5 text-xs">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-bold" id="billSubtotal">{{ $shopCurrency }} 0</span>
                    </div>

                    <!-- Discount Input row -->
                    <div class="flex justify-between items-center">
                        <span class="text-slate-400 font-medium">Potongan Diskon ({{ $shopCurrency }})</span>
                    ...
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="font-extrabold text-indigo-600 dark:text-indigo-400" id="billGrandTotal">{{ $shopCurrency }} 0</span>
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-1">
                        ...
                        <span class="font-bold text-slate-700 dark:text-slate-300" id="billChangeAmount">{{ $shopCurrency }} 0</span>
                    </div>
...
<div class="hidden">
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            window.open("{{ route('transactions.receipt', session('print_receipt_id')) }}", "Cetak Struk", "width=400,height=600,scrollbars=yes");
        });
    </script>
</div>
@endif
